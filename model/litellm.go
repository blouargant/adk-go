...
const (
	// LiteLLMDefaultModel is the default model name for LiteLLM.
	LiteLLMDefaultModel = "gpt-4o"

	// EnvLiteLLMAPIKey is the environment variable name for the LiteLLM API key.
	EnvLiteLLMAPIKey = "LITELLM_API_KEY"

	// EnvLiteLLMBaseURL is the environment variable name for the LiteLLM base URL.
	EnvLiteLLMBaseURL = "LITELLM_BASE_URL"
)

// LiteLLM represents a LiteLLM Large Language Model.
// LiteLLM uses Bearer token authentication instead of OpenAI's standard API key authentication.
type LiteLLM struct {
	*BaseLLM
...
var _ types.Model = (*LiteLLM)(nil)

// NewLiteLLM creates a new [LiteLLM] instance.
// Unlike OpenAI, LiteLLM uses Bearer token authentication via Authorization header.
func NewLiteLLM(ctx context.Context, apiKey, baseURL, modelName string, opts ...Option) (*LiteLLM, error) {
	// Use default model if none provided
	if modelName == "" {
		modelName = LiteLLMDefaultModel
	}

	// Check API key and use [EnvLiteLLMAPIKey] environment variable if not provided
	if apiKey == "" {
		envApiKey := os.Getenv(EnvLiteLLMAPIKey)
		if envApiKey == "" {
			return nil, fmt.Errorf("either apiKey arg or %q environment variable must be set", EnvLiteLLMAPIKey)
		}
		apiKey = envApiKey
	}

	// Setup client options - NOTE: We don't use WithAPIKey for LiteLLM
	var clientOpts []option.RequestOption

	// Set Authorization header with Bearer token instead of using WithAPIKey
	bearerToken := fmt.Sprintf("Bearer %s", apiKey)
	clientOpts = append(clientOpts, option.WithHeaderAdd("Authorization", bearerToken))

	// Check base URL and use environment variable if not provided
	if baseURL == "" {
		baseURL = os.Getenv(EnvLiteLLMBaseURL)
	}
	if baseURL != "" {
	...
	// Create OpenAI-compatible client for LiteLLM
	...
	return litellmClient, nil
}

// Name returns the name of the [LiteLLM] model.
func (m *LiteLLM) Name() string {
	return m.modelName
}

// SupportedModels returns a list of supported models in LiteLLM.
// LiteLLM supports a wide range of models from different providers.
//
// See https://docs.litellm.ai/docs/providers for complete list.
func (m *LiteLLM) SupportedModels() []string {
	return []string{
		// OpenAI models
		"gpt-4o",
		"gpt-4o-mini",
...
// Connect creates a live connection to the LiteLLM LLM.
// Note: LiteLLM doesn't support live connections, so this returns an error.
func (m *LiteLLM) Connect(ctx context.Context, _ *types.LLMRequest) (types.ModelConnection, error) {
	return nil, types.NotImplementedError("LiteLLM: Live connection is not supported")
}

// GenerateContent generates content from the model.
func (m *LiteLLM) GenerateContent(ctx context.Context, request *types.LLMRequest) (*types.LLMResponse, error) {
...
func (m *LiteLLM) StreamGenerateContent(ctx context.Context, request *types.LLMRequest) iter.Seq2[*types.LLMResponse, error] {
...
func (m *LiteLLM) convertContentsToMessages(contents []*genai.Content) ([]openai.ChatCompletionMessageParamUnion, error) {
...
func (m *LiteLLM) convertContentToMessage(content *genai.Content) (openai.ChatCompletionMessageParamUnion, error) {
...
func (m *LiteLLM) extractTextFromParts(parts []*genai.Part) string {
...
func (m *LiteLLM) convertGenaiToolsToOpenAI(tools []*genai.Tool) ([]openai.ChatCompletionToolParam, error) {
...
func (m *LiteLLM) convertResponseToLLMResponse(response *openai.ChatCompletion) (*types.LLMResponse, error) {
	if len(response.Choices) == 0 {
		return &types.LLMResponse{
			ErrorCode:    "NO_CHOICES",
			ErrorMessage: "LiteLLM response contains no choices",
		}, nil
	}
...
func (m *LiteLLM) convertChunkToLLMResponse(chunk openai.ChatCompletionChunk) (*types.LLMResponse, error) {
...
func (m *LiteLLM) containsText(r *types.LLMResponse) bool {
...
func (m *LiteLLM) newAggregateText(text string) *types.LLMResponse {
	return &types.LLMResponse{
		Content: &genai.Content{
			Role:  RoleModel,
...
func (m *LiteLLM) isStreamFinished(chunk *openai.ChatCompletionChunk) bool {
	if len(chunk.Choices) == 0 {
		return false
	}
...
func (m *LiteLLM) buildResponseLog(response *openai.ChatCompletion) slog.Attr {
	if len(response.Choices) == 0 {
